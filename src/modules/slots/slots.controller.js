...
const release = asyncHandler(async (req, res) => { const assignment = await slotsService.releaseSlotAssignment(req.params.assignmentId); ApiResponse.success(res, 'Slot assignment released', assignment); });
const analytics = asyncHandler(async (req, res) => { const data = await slotsService.getSlotAnalytics(req.query); ApiResponse.success(res, 'Slot analytics fetched', data); });
...
export { createInventory, getInventory, assign, release, analytics, getMySlots, getFeatured, getAll };
